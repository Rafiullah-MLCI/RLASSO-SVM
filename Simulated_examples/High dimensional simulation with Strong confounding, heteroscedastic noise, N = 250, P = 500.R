######################################
# RLASO_SVM simulation with Strong confounding, heteroscedastic noise#
######################################
...
get_features <- function(N, P) {
  ...
  x = matrix(NA, N, P)
  x[, 1:200] = qnorm(unif[, 1:200])
  x[, 201:P] = qbinom(unif[, 201:P], 1, 0.3)
...
### INFO
N =250
P = 500  # We tried P = 500 to see how the problem degenerate
B = 500

# Store Metrics
Bias_XLasso = c(NA); Bias_ULasso = c(NA);   Bias_SLasso = c(NA);Bias_RLASSO = c(NA); Bias_TLasso = c(NA); Bias_CF = c(NA)
PEHE_XLasso = c(NA); PEHE_ULasso = c(NA);  PEHE_RLASSO = c(NA) ; PEHE_SLasso = c(NA); PEHE_TLasso = c(NA);  PEHE_CF = c(NA)
PEHE_SBART = c(NA); Bias_SBART = c(NA); PEHE_RLASSO_SVM = c(NA); Bias_RLASSO_SVM = c(NA)
...
for (b in 1:B) {
  ...
  Pscore = 0.9*plogis(1.2 - 2*mu_base)   # stronger confounding
  w <- rbinom(N, 1, Pscore)
  y = mu + ITE*w + rnorm(N, 0, (abs(x[, 5])+1)*0.5)   # heteroscedastic noise
...
ggplot(PEHE_summary, aes(x = reorder(Model, mean_PEHE), y = mean_PEHE, group = 1)) +
  ...
  labs(title = "Sample Size N = 250, Number of Covariates P = 500", x = "Model", y = "Mean PEHE") +
...
ggplot(Bias_summary, aes(x = reorder(Model, mean_Bias), y = mean_Bias, group = 1)) +
  geom_line() + geom_point() +
  labs(title = "Sample Size N = 250, Number of Covariates P = 500", x = "Model", y = "Mean Bias") +
  theme_minimal() +theme(plot.title = element_text(face = "italic", size = 10))+ coord_flip()
...
ggplot(PEHE_long, aes(x = Model, y = PEHE, fill = Model)) +
  geom_boxplot() +
  labs(title = "Sample Size N = 250, Number of Covariates P = 500", x = "Model", y = "PEHE") +
  theme_minimal() +
...
ggplot(Bias_long, aes(x = Model, y = Bias, fill = Model)) +
  geom_boxplot() +
  labs(title = "Sample Size N = 250, Number of Covariates P = 500", x = "Model", y = "Bias") +
  theme_minimal() +
  theme(plot.title = element_text(face = "italic", size = 10), axis.text.x = element_text(angle = 45, hjust = 1))
