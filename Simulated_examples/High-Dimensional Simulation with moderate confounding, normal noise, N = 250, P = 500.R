...
get_features <- function(N, P) {
  ...
  ###S Transform in continuous and binary covariates-----------------------------
  x = matrix(NA, N, P)
...
for (b in 1:B) {
  ...
  ######################### R-LASSO-SVM
  Rlasso_svm <- rlasso_svm(x = x, w = w, y = y, type="nu-regression", kernel="polynomial", cost= 500, gamma = 0.01, epsilon = 0.01)
